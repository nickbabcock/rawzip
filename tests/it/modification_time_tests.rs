...
fn test_no_modification_time_defaults_to_zero() {
    ...
    {
        let mut archive = ZipArchiveWriter::new(&mut output);
        let (mut entry, content_builder) = archive.new_file("test.txt").start().unwrap();
        let mut writer = content_builder.wrap(&mut entry);
        writer.write_all(b"Hello, world!").unwrap();
        let (_, descriptor) = writer.finish().unwrap();
        entry.finish(descriptor).unwrap();
...
fn test_multiple_files_different_timestamps() {
    ...
    // Create archive with multiple files having different timestamps
    {
        ...
        // First file
        let (mut entry1, content_builder1) = archive
            .new_file("file1.txt")
            .last_modified(datetime1)
            .start()
            .unwrap();
        let mut writer1 = content_builder1.wrap(&mut entry1);
        writer1.write_all(b"File 1").unwrap();
        let (_, descriptor1) = writer1.finish().unwrap();
        entry1.finish(descriptor1).unwrap();

        // Second file
        let (mut entry2, content_builder2) = archive
            .new_file("file2.txt")
            .last_modified(datetime2)
            .start()
            .unwrap();
        let mut writer2 = content_builder2.wrap(&mut entry2);
